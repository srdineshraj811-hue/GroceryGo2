...
export default function ShopperDashboard() {
  ...
  const handleTogglePurchased = (itemKey: string) => {
          ...
          <TabsContent value="purchase" className="mt-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between mb-4">
                ...
                <p className="text-sm text-muted-foreground">{purchaseList.length} items</p>
              </div>
              {purchaseList.map((item, index) => {
                const itemKey = `${item.deliveryId}-${item.id}-${index}`;
                const isPurchased = purchasedItems[itemKey];
                return (
                  <Card
                    key={itemKey}
                    className={`p-4 ${isPurchased ? 'opacity-50' : ''}`}
                    data-testid={`purchase-item-${itemKey}`}
                  >
                    <div className="flex items-start gap-3">
                      <Checkbox
                        checked={isPurchased}
                        onCheckedChange={() => handleTogglePurchased(itemKey)}
                        className="mt-1"
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              Qty: {item.quantity} • Order #{item.orderNumber}
                            </p>
